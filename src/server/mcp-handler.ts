        ...
        tools: [
          {
            ...
            description: 'Load an MCP server into a secure isolated Worker environment for code mode execution. Can use a saved configuration or load with a new configuration. Automatically saves the configuration unless auto_save is false.',
            inputSchema: {
              type: 'object',
              properties: {
                mcp_name: {
                ...
                mcp_config: {
                  type: 'object',
                  description: 'MCP server connection configuration. Required if use_saved is false. Can use ${VAR_NAME} syntax for environment variables.',
                  properties: {
                    command: {
                      type: 'string',
                    ...
                    env: {
                      type: 'object',
                      description: 'Environment variables for the MCP server. Use ${VAR_NAME} syntax to reference .env variables.',
                    },
                  },
                  required: ['command'],
                },
...
Use console.log() to output results - all console output is captured and returned in the response.`,
            inputSchema: {
              ...
              properties: {
                mcp_id: {
                  type: 'string',
                  description: 'UUID of the loaded MCP server (returned from load_mcp_server). Use list_available_mcps or get_mcp_by_name to find the MCP ID.',
                },
                code: {
                  type: 'string',
...
The code runs in an isolated Worker environment with no network access. All MCP communication happens through the 'mcp' binding object.`,
          ...
          },
          {
            ...
            description: 'List all MCP servers currently loaded in Worker isolates. Returns a list with MCP IDs, names, status, and tool counts. Use get_mcp_by_name to find a specific MCP by name.',
            inputSchema: {
              type: 'object',
              properties: {},
            },
          },
          {
            ...
            description: 'Find a loaded MCP server by its name. Returns the MCP ID and basic information for quick lookup. This is more efficient than calling list_available_mcps and searching manually.',
            inputSchema: {
              type: 'object',
              properties: {
                mcp_name: {
          ...
          },
          {
            ...
            description: 'Unload an MCP server and clean up its Worker isolate. Optionally remove from saved configurations.',
            inputSchema: {
              type: 'object',
              properties: {
                mcp_id: {
          ...
          },
          {
            ...
            description: 'List all saved MCP configurations from Cursor\'s MCP configuration file',
            inputSchema: {
              type: 'object',
              properties: {},
            },
          },
          {
            ...
            description: 'Save an MCP configuration to Cursor\'s MCP configuration file. Configurations are saved in JSONC format with environment variable placeholders.',
            inputSchema: {
              type: 'object',
              properties: {
                mcp_name: {
                ...
                mcp_config: {
                  type: 'object',
                  description: 'MCP server configuration to save. Use ${VAR_NAME} syntax for environment variables.',
                  properties: {
                    command: {
                      type: 'string',
                    ...
                    env: {
                      type: 'object',
                      description: 'Environment variables. Use ${VAR_NAME} syntax to reference .env variables.',
                    },
                  },
                  required: ['command'],
                },
          ...
          },
          {
            ...
            description: 'Delete a saved MCP configuration from Cursor\'s MCP configuration file',
            inputSchema: {
              type: 'object',
              properties: {
                mcp_name: {
  ...
  private async handleLoadMCP(args: any) {
    ...
    if (use_saved) {
      ...
      const validated = validateInput(LoadMCPRequestSchema, { mcp_name, mcp_config });
  ...
  private async handleExecuteCode(args: any) {
    ...
    if (!instance) {
      throw new MCPIsolateError(
        ...
        'NOT_FOUND',
        404,
        { mcp_id: validated.mcp_id, suggestion: 'Use list_available_mcps or get_mcp_by_name to find the correct MCP ID' }
      );
    }
  ...
  private getExecutionErrorSuggestion(error?: string): string {
    ...
    if (errorLower.includes('not defined') || errorLower.includes('undefined')) {
  ...
  private getSuggestedAction(errorCode: string, toolName: string): string {
    ...
    return suggestions[errorCode] || 'Review the error details and try again. If the issue persists, check the logs.';
  }
...
Available tools: ${tools.map(t => t.name).join(', ')}`;
  ...
  private generateExampleCode(tools: MCPTool[]): string {
    ...
    const paramKeys = required.length > 0 ? required.slice(0, 2) : Object.keys(params).slice(0, 2);
    const exampleParams: Record<string, any> = {};
    
    paramKeys.forEach(key => {
      const schema = params[key];
      if (schema.type === 'string') {
        exampleParams[key] = schema.description?.toLowerCase().includes('query') ? 'typescript' : 'example';
      } else if (schema.type === 'number') {
        exampleParams[key] = 1;
      } else if (schema.type === 'boolean') {
        exampleParams[key] = true;

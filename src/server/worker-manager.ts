...
export class WorkerManager {
  ...
  private async checkWranglerAvailability(): Promise<void> {
    try {
      ...
      logger.warn('Wrangler not available via npx. Worker execution will be simulated.');
    }
  }
  ...
  private startRPCServer(): void {
    ...
    this.rpcServer = createServer(async (req: IncomingMessage, res: ServerResponse) => {
      ...
      try {
        ...
        if (!client) {
          res.writeHead(404, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ error: `MCP client not found for ID: ${mcpId}` }));
          return;
        }
        ...
        logger.error({ error: error.message, stack: error.stack }, 'RPC: Error calling MCP tool');
  ...
  async loadMCP(mcpName: string, config: MCPConfig): Promise<MCPInstance> {
    ...
    try {
      ...
      const hasCachedSchema = cached && cached.configHash === this.hashConfig(mcpName, config);
      ...
      if (hasCachedSchema) {
        ...
        logger.debug({ mcpId, mcpName, cacheKey, toolCount: tools.length }, 'Cached MCP schema');
      }
  ...
  private async startMCPProcess(config: MCPConfig, hasCachedSchema: boolean = false): Promise<ChildProcess> {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        logger.debug({ pid: mcpProcess.pid }, 'MCP process spawned successfully');
        ...
        reject(new MCPConnectionError(`Failed to spawn MCP process: ${spawnError.message}`));
      }
    });
  }

  private async fetchMCPSchema(mcpName: string, config: MCPConfig, mcpId: string): Promise<MCPTool[]> {
    ...
    try {
      ...
      logger.debug({ mcpId, mcpName, toolCount: toolsResponse.tools.length, listToolsTimeMs: listToolsTime }, 'Fetched tools from MCP server');
  ...
  private async generateWorkerCode(
    ...
    const escapedRpcUrl = rpcUrl.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$/g, '\\$');
    const escapedMcpId = mcpId.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$/g, '\\$');
    
    const mcpBindingStubs = tools.map(tool => {
...
`;
  ...
  private async executeInIsolate(
    ...
    if (this.wranglerAvailable) {
      try {
        ...
        // Don't fall back to simulation - always throw the error so the user knows execution failed
        // The error will be caught by executeCode() and returned with success: false
        throw error;
      }
    }
    ...
    return {
      output: `Simulated execution result for code (${code.length} chars)\n` +
  ...
  private async executeWithWrangler(
    ...
    try {
      ...
      const workerCode = await this.generateWorkerCode(mcpId, instance.tools, instance.typescript_api, code);
      ...
      await new Promise<void>((resolve, reject) => {
          ...
          try {
            const response = await fetch(`http://localhost:${port}`, {
              ...
              body: JSON.stringify({ 
                ...
                workerCode: { compatibilityDate: '2025-06-01', mainModule: 'test.js', modules: { 'test.js': 'export default { fetch: () => new Response("ok") }' } },
            ...
            if (checkCount < maxChecks && !error.name?.includes('AbortError')) {
              ...
              clearTimeout(timeout);
              const healthCheckError = new Error(`Wrangler health check failed after ${maxChecks} attempts. Last error: ${error.message}`);
        ...
        if (wranglerProcess?.stdout) {
          wranglerProcess.stdout.on('data', (data: Buffer) => {
            const output = data.toString();
            if ((output.includes('Ready') || output.includes('ready') || output.includes('Listening')) && !ready) {
              ready = true;
              clearTimeout(timeout);
              if (isCLIMode) {
                progress.updateStep(1, 'success');
          ...
          if (!ready && code !== null && code !== 0) {
            ...
            if (hasWorkerLoadersError) {
              const error = new Error(
                ...
                'Error details: ' + (wranglerStderr || wranglerStdout).split('\n').find(line => line.includes('worker_loaders')) || 'Unknown error'
              );
      ...
      if (isCLIMode) {
        ...
        const hasWorkerLoadersError = (wranglerStderr?.includes('worker_loaders') || 
                                     wranglerStdout?.includes('worker_loaders') ||
                                     error.message?.includes('worker_loaders')) ?? false;
        const hasBuildError = wranglerStderr.includes('Build failed') || 
                             wranglerStderr.includes('build failed') ||
                             wranglerStderr.includes('✗ Build failed') ||
        ...
        if (hasWorkerLoadersError || hasBuildError || 
        ...
        } else if (error.message.includes('Worker execution failed') || 
                   error.message.includes('execute') ||
                   (error.message.includes('fetch') && error.message.includes('localhost'))) {
      ...
      console.error('\n' + formatWranglerError(error, wranglerStdout || '', wranglerStderr || '', context) + '\n');

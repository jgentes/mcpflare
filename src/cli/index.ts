...
import { LoadMCPRequestSchema, ExecuteCodeRequestSchema } from '../types/mcp.js';
...
async function loadMCP() {
  try {
    ...
    if (savedNames.length > 0) {
      ...
      if (useSavedLower !== 'new' && useSavedLower !== String(savedNames.length + 1)) {
        // User wants to use a saved config
        let selectedName: string | null = null;
        const selectedNum = parseInt(useSavedLower, 10);
        
        if (!isNaN(selectedNum) && selectedNum >= 1 && selectedNum <= savedNames.length) {
          selectedName = savedNames[selectedNum - 1];
        } else {
          // Try to find by name
          selectedName = savedNames.find(name => name.toLowerCase() === useSavedLower) || null;
        }
    ...
    const args = argsInput.trim() ? argsInput.split(',').map(s => s.trim()) : [];
    ...
    if (conflictingMCP) {
      ...
      console.log(`   If you're using mcp-isolate, consider disabling "${mcpName}" in your IDE's MCP settings`);
      console.log(`   to avoid confusion. The IDE will use the real MCP, while mcp-isolate uses the sandboxed version.`);
...
async function testTool() {
  try {
    ...
    if (!isNaN(selectionNum) && selectionNum >= 1 && selectionNum <= allMCPs.length) {
      // User selected by number
      selectedMCP = allMCPs[selectionNum - 1];
    } else {
      // User entered name
      const searchTerm = selection.trim().toLowerCase();
      selectedMCP = allMCPs.find(
        mcp => mcp.name.toLowerCase() === searchTerm
      );
...
async function executeCode() {
  try {
    ...
    if (!isNaN(selectionNum) && selectionNum >= 1 && selectionNum <= instances.length) {
      // User selected by number
      ...
      selectedInstance = instances.find(
        inst => inst.mcp_id.toLowerCase() === searchTerm || 
...
async function checkIDEConflicts() {
  ...
  conflicts.forEach(conflict => {
    if (conflict.inIDE && conflict.inIsolate) {
      hasConflicts = true;
      console.log(`⚠️  "${conflict.name}" is configured in both your IDE and mcp-isolate`);
      console.log(`   Recommendation: Disable "${conflict.name}" in your IDE's MCP settings`);
      console.log(`   to avoid confusion. The IDE will use the real MCP, while mcp-isolate`);
      console.log(`   uses the sandboxed version.\n`);
    } else if (conflict.inIDE && !conflict.inIsolate) {
      console.log(`ℹ️  "${conflict.name}" is configured in your IDE but not loaded in mcp-isolate`);
...
async function deleteSavedConfig() {
  ...
  if (!isNaN(selectionNum) && selectionNum >= 1 && selectionNum <= savedNames.length) {
    selectedName = savedNames[selectionNum - 1];
  } else {
    selectedName = savedNames.find(name => name.toLowerCase() === selection.trim().toLowerCase()) || null;
  }

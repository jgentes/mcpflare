...
export const DEFAULT_SECURITY_CONFIG: Omit<
  MCPSecurityConfig,
  'id' | 'mcpName' | 'isGuarded' | 'lastModified'
> = {
  network: {
...
> = {
  legitimateCall: {
    ...
    description:
      'Verifies that normal MCP tool calls work correctly when guarded',
    icon: 'check',
  },
  networkIsolation: {
    name: 'Network Isolation',
    description:
      'Tests that arbitrary network requests (fetch, HTTP) are blocked',
    icon: 'network',
  },
  codeInjection: {
    name: 'Code Injection Prevention',
    description:
      'Tests that dangerous code patterns (eval, Function) are blocked',
    icon: 'shield',
  },
  filesystemIsolation: {

...
export class ConfigManager {
    ...
    getAllConfiguredMCPs() {
        ...
        for (const [name, config] of Object.entries(rawConfig._mcpflare_disabled || {})) {
            if (name.toLowerCase() !== 'mcpflare' && config) {
                allMCPs[name] = {
                    config: config,
                    source: this.configSource,
    ...
    disableMCP(mcpName) {
        ...
        const ide = this.ideDefinitions.find((d) => d.id === this.configSource);
        const sourceName = ide ? ide.displayName : 'IDE';
        logger.info({ mcpName, configPath: this.configPath, source: this.configSource }, `MCP disabled in ${sourceName} config file (moved to _mcpflare_disabled)`);
        return true;
    }
    enableMCP(mcpName) {
        if (!this.configPath) {
        ...
        const ide = this.ideDefinitions.find((d) => d.id === this.configSource);
        const sourceName = ide ? ide.displayName : 'IDE';
        logger.info({ mcpName, configPath: this.configPath, source: this.configSource }, `MCP enabled in ${sourceName} config file (moved from _mcpflare_disabled)`);
        return true;
    }

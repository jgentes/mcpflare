...
description: Using MCPflare as an MCP server for AI agents.
---
...
Add MCPflare to your IDE's MCP configuration:
...
When MCPflare discovers your other MCPs, their tools become available with namespaced names:
...
MCPflare also supports **MCP Prompts** - pre-defined message templates that appear as slash commands in your IDE (e.g., `/mcpflare/github:AssignCodingAgent`).
...
Prompts don't execute any code - they just return pre-defined messages. This makes them safe by design, so MCPflare proxies them directly to the underlying MCP without the overhead of worker isolation. Only tools that execute actions require the security of worker isolates.

**Example prompts:**
```
mcpflare/github:AssignCodingAgent
mcpflare/github:issue_to_fix_workflow
```

The `mcpflare/github:` prefix shows these are GitHub MCP prompts routed through MCPflare's transparent proxy.
...
By default, MCPflare operates in **transparent proxy mode**:
...
<Callout type="info" title="No Config Changes Needed">
  Once MCPflare is running, all your existing MCP tool calls automatically go through secure isolation. The AI doesn't need to know about the isolation layer.
</Callout>
...
1. MCPflare intercepts the namespaced call
...
- MCPflare still discovers disabled MCPs
